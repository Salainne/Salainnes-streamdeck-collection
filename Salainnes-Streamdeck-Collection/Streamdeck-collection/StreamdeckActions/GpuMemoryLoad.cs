...
{
    [StreamdeckAction("com.salainne.salainnes-streamdeck-collection.GpuMemoryLoad")]
    public static class GpuMemoryLoad
    {
        public static void Run(StreamDeckConnection connection, KeyValuePair<string, ActiveIconContainer> icon)
        {
            var load = Helpers.SystemMonitor.GetVramUsagePercent();

            var tmp = $"VRAM{Environment.NewLine}{load:####}%";
